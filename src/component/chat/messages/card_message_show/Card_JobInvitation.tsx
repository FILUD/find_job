        ...
        <div className='card max-w-96  w-full bg-primary text-info-content glass font-notoLao'>
            ...
            {type == "employer" ? (
                < div className="card-body">
                    <h2 className="card-title justify-center">ຄຳເຊີນເຂົ້າຮ່ວມວຽກ</h2>
                    {data.Status == "Pending" ? (
                        <div>
                    ...
                    <div >
                        {data.Status == "Pending" ? (
                            ...
                            <div className='flex space-x-4 justify-center'>
                                <button className='btn hover:btn-outline  ' onClick={() => toggleView()}>ເບິ່ງລາຍລະອຽດ</button>
                                <button className='btn hover:btn-success btn-success'>ຖືກຕອບຮັບແລ້ວ</button>
                            </div>
                        )}
                    </div>
                ...
                < div className="card-body">
                    ...
                    {data.Status == "Pending" ? (
                        <div>
                            <p className='self-center'>ທາ່ນຕ້ອງການຕອບຮັບຄຳເຊີນເຈົ້າຮ່ວມວຽກນີ້ບໍ່ ? </p>
                            <p> ອາຊີບ : {data.OccupationName}</p>
                            <p className='line-clamp-1'>ໝວດຫມູ່ອາຊີບ : {data.CategoryName} </p>
                        </div>
                    ...
                    <div >
                        {data.Status == "Pending" ? (
                            <div className='flex space-x-4 justify-center'>
                                <button className='btn hover:btn-outline  ' onClick={() => toggleView()}>ເບິ່ງລາຍລະອຽດ</button>
                                <button className='btn hover:btn-outline' onClick={() => handleAcceptJobInvitation()}>ຍອມຮັບຄຳເຊີນ</button>
                            </div>
                        ) : (
                            <div className='flex space-x-4 justify-center'>
                                <button className='btn hover:btn-outline  ' onClick={() => toggleView()}>ເບິ່ງລາຍລະອຽດ</button>
