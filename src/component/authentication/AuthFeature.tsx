    ...
    const saveRegister = async (requireSignUpEmail: string) => {
        try {
            const response = await fetch(`${api}/register`, {
                ...
                body: JSON.stringify({ email: requireSignUpEmail, password: passwordSignup }),
            });
                                        ...
                                        {isLoading ? (
                                            ...
                                            <div className="card-body">
                                                ...
                                                <label className={Authcss.labelInput}>
                                                    ...
                                                    <input type={inputType} className="grow" placeholder="Password" value={passwordSignup} onChange={(e) => setPasswordSignup(e.target.value)} />
                                                    <kbd className="kbd kbd-sm cursor-pointer" onClick={togglePasswordVisibility}>
                                                        {showPassword ?
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-4 h-4 hover:scale-125">
                                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                                    ...
                                                    <div className="mt-2 grid justify-items-center space-y-4">
                                                        ...
                                                        <div className='card-actions justify-center mt-2 w-full '>
                                                            <button className="btn btn-primary rounded-2xl btn-wide btn-error hover:text-white  text-base "
                                                                onClick={() => handleSignup(emailSignup)}>Register</button>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>)}
